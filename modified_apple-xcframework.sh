...
#export VISIONOS_PREFIX="${PREFIX}/tmp/visionos"                                  # NOTE(pyoung): vision - not supported yet
#export VISIONOS_SIMULATOR_PREFIX="${PREFIX}/tmp/visionos-simulator"              # NOTE(pyoung): vision - not supported yet
...
# echo "Building for visionOS..."                    # NOTE(pyoung): vision - not supported yet
# build_visionos >"$LOG_FILE" 2>&1 || exit 1         # NOTE(pyoung): vision - not supported yet
...
if [ -z "$LIBSODIUM_SKIP_SIMULATORS" ]; then
  ...
  # echo "Building for the visionOS simulator..."            # NOTE(pyoung): vision - not supported yet
  # build_visionos_simulator >"$LOG_FILE" 2>&1 || exit 1     # NOTE(pyoung): vision - not supported yet
else
...
# NOTE(pyoung): vision - not supported yet
